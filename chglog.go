...
type Options struct {
	...
	TagSortBy                   string              // Specify how to sort tags; currently supports "date" (default) or by semver.
...
func NewGenerator(logger *Logger, config *Config) *Generator {
	...
	return &Generator{
		client:          client,
		config:          config,
		tagReader:       newTagReader(client, config.Options.TagFilterPattern, config.Options.TagSortBy),
